...
(restas:define-route login ("login")
  (finalize-page (rulisp.auth.view:login `(:forgot-href ,(restas:genurl 'forgot)))
                 "Вход"))
...
(restas:define-route register ("register")
  (:requirement #'not-logged-on-p)
  (finalize-page (rulisp.auth.view:register-form nil)
                 "Регистрация"))
...
(restas:define-route register/post ("register" :method :post)
  ...
  (let ((form-bads (check-register-form))
    ...
    (finalize-page (if form-bads
                       (rulisp.auth.view:register-form (list* :name login
							      :email email
							      :password password
                       ...
                       (let ((invite (create-invite login email (password-cache password)))
                             (to (list email)))
                         (send-mail to
                                    (rulisp.auth.view:confirmation-mail
                                     (list :to to
                         ...
                         (rulisp.auth.view:register-send-mail nil)))
                   "Регистрация")))
...
(restas:define-route accept-invitation ("register/confirmation/:(invite)")
  ...
  (if (invite-exist-p invite)
	...
	(run-login (first account)
		   (third account))
	(finalize-page (rulisp.auth.view:success-registration nil)
...
(restas:define-route forgot ("forgot")
  (:requirement #'not-logged-on-p)
  (finalize-page (rulisp.auth.view:forgot nil)
          ...
          (if mark
              (progn
                (send-mail (list email)
                           (rulisp.auth.view:forgot-mail (list :to (list email)
							       :noreply-mail *noreply-email*
							       :subject (prepare-subject "Восстановление пароля")
							       :host (hunchentoot:host)
							       :link (restas:genurl* 'reset-password :mark mark))))
                (finalize-page (rulisp.auth.view:forgot-send-mail nil)
                               "Восстановление пароля"))
              (finalize-page (rulisp.auth.view:forgot (list :bad t))
                             "Восстановление пароля"))))))

(restas:define-route reset-password ("reset-password/:(mark)")
  (:requirement #'not-logged-on-p)
  (if (forgot-mark-exist-p mark)
      (finalize-page (rulisp.auth.view:reset-password-form nil)
                     "Изменение пароля")
      hunchentoot:+HTTP-NOT-FOUND+))
  ...
  (if (forgot-mark-exist-p mark)
      (let ((bads nil))
        ...
        (finalize-page (if bads 
                           (rulisp.auth.view:reset-password-form (list* :password (form-field-value "password")
									:re-password (form-field-value "re-password")
									bads))
                           (progn (change-password mark
                                                   (password-cache (hunchentoot:post-parameter "password")))
                                  (rulisp.auth.view:reset-password-success nil)))
                       "Изменение пароля"))
      (restas:redirect 'forgot)))
